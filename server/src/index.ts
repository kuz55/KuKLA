...
// Handlers are registered without explicit route generics, so Fastify infers
// `Params` as `unknown`. Widening the parameter keeps every call site usable.
const parseId = (req: FastifyRequest) => z.string().uuid().parse((req.params as Params).id);
